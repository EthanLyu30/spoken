...
def save_practice(payload: PracticeCreate, db: Session = Depends(get_db)) -> PracticeRecordOut:
    return PracticeRecordOut.model_validate(repo.create_record(db, payload))


@router.get("/practice", response_model=list[PracticeRecordOut])
def list_practice(
    kind: str | None = None, limit: int = 100, db: Session = Depends(get_db)
) -> list[PracticeRecordOut]:
    return [PracticeRecordOut.model_validate(r) for r in repo.list_records(db, kind, limit)]


@router.get("/stats", response_model=Stats)
def stats(db: Session = Depends(get_db)) -> Stats:
    return Stats(**repo.compute_stats(db))

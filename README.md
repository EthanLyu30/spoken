...
- **语音流畅与低延迟** —— 朗读默认用浏览器原生语音（连贯自然、几乎即时），讯飞云端可选；对话回复流式输出；通话模式 ASR 实时流式（边说边出字幕，说完即接话）。
...
| 🎙️ 实时语音对话 | 通话模式（能量 VAD 自动断句 + 实时字幕 + 断流重连）/ 按住说话（识别后自动发送，无需再点发送）：讯飞 ASR → DeepSeek → 朗读 |
...
> 当前实现：对话回复已**流式**；通话模式下**朗读逐句流式**（首句一生成就开始播放，其余边生成边合成，明显缩短跟手延迟），且 **ASR 实时流式**（边说边出字幕，说完即用最终结果接话；连不上中继时自动回退缓冲转写）；朗读默认浏览器原生语音、可切讯飞。完整设计见 **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**。

...
> 占位：运行后将以下界面截图放入 `docs/screenshots/`，并替换下方链接即可在此展示。

| 首页闯关地图 | 场景对话（流式） | 课后小结 |
|---|---|---|
| `docs/screenshots/home.png` | `docs/screenshots/chat.png` | `docs/screenshots/report.png` |

| 每日金句（出处+场景） | 限时问答 | 我的（成长曲线） |
|---|---|---|
| `docs/screenshots/daily.png` | `docs/screenshots/interview.png` | `docs/screenshots/profile.png` |
...
```
              ┌──────────── 快路（低延迟，保证对话流畅） ───────────┐
 🎤 麦克风 ─VAD断句─► ASR 转写 ──► 对话 LLM(流式) ──► 朗读(浏览器/讯飞) ─► 🔊
                      │
                      └─► 转写文本 ─┐
                                    ├─► 慢路（异步，不阻塞对话）
 对话结束 ───────────────────────────┴─► 发音评测 + 语法纠错 + 课后总结 ─► 📊
...
> 当前实现：对话回复已**流式**；朗读默认浏览器原生语音、可切讯飞；ASR 为缓冲转写（逐句流式为后续优化项）。完整设计见 **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**。

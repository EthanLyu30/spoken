...
| 🎙️ 实时语音对话 | 通话模式（能量 VAD 自动断句 + 断流重连）/ 按住说话（识别后自动发送，无需再点发送）：讯飞 ASR → DeepSeek → 朗读 |
...
> 当前实现：对话回复已**流式**；通话模式下**朗读逐句流式**（首句一生成就开始播放，其余边生成边合成，明显缩短跟手延迟）；朗读默认浏览器原生语音、可切讯飞；ASR 仍为缓冲转写（逐句流式 ASR 为后续优化项）。完整设计见 **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**。
